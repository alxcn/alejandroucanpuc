...
El movimiento de un dron a lo largo de una dirección (digamos, la posición $x(t)$) bajo la influencia de una fuerza de control $u(t)$ puede ser modelado por una EDO de primer orden: $$x'(t)+ax(t)=bu(t)$$ donde $x(t)$ es la posición y $a,b$ son constantes que dependen de las características del dron (resistencia al aire, masa, etc).

--

### Ejemplo:

> Supongamos que un dron se mueve en una dirección y la fuerza de control es $u(t)=e^{-t}.$ Si la posición inicial es $x(0)=1.$ Encuentra la posición del dron en el tiempo.

---
# Simulación de redes de colas

Supongamos que $q(t)$ representa el número de solicitudes en cola en un servidor en un instante $t$, y $\lambda(t)$ es la tasa a la cual llegan nuevas solicitudes. Una posible EDO sería: $$q'(t)+\mu q(t)=\lambda(t)$$ donde $\mu$ es la tasa en que el servidor procesa un servicio y $\lambda(t)$ es la tasa de llegada de solicitudes.

--

## Ejemplo 2
El número de solicitudes en cola en un servidor web q(t) cuenta con una tasa de 4 solicitudes por minuto, y se cuenta con una tasa de nuevas solicitudes de 10 solicitudes por minuto.
Si inicialmente no hay solicitudes en cola, ¿cuál es el número de solicitudes en cola en el tiempo t?
